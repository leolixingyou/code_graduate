...
class Drawer(object):
    ...
    def draw(self, show_img, dets, labels, dists, ori_dist_arr,mul_dist,combine_dist, lidar_dist, fps, is_tracker=True):
        ...
        for idx in range(dets.shape[0]):
            ...
            if height < 50:
                ...
                lidar_dist_position = (dets[idx, 0], dets[idx, 3] + offset+20)
                mul_dist_position = (dets[idx, 0], dets[idx, 3] + offset+25)
                                    
                if len(dists) > 0 and dists is not None:
                    # for i, dist in enumerate(dists):
                    ##DRAW (DIST)ori_dist_arr
                    cv2.putText(show_img, str(dists[dist_ct]), dist_position, cv2.FONT_HERSHEY_SIMPLEX, self.fontSize*0.25, (0, 255, 0),thickness=2)
                    cv2.putText(show_img, str(ori_dist_arr[dist_ct]), ori_dist_position, cv2.FONT_HERSHEY_SIMPLEX, self.fontSize*0.25, (0, 0, 255),thickness=2)
                    cv2.putText(show_img, str(lidar_dist), lidar_dist_position, cv2.FONT_HERSHEY_SIMPLEX, self.fontSize*0.25, (255, 0, 0),thickness=2)
                    cv2.putText(show_img, str(mul_dist), mul_dist_position, cv2.FONT_HERSHEY_SIMPLEX, self.fontSize*0.25, (255, 0, 255),thickness=2)
                    dist_ct+=1
                ...
                mul_dist_position = (dets[idx, 0], dets[idx, 3] + offset+130)
                # combine_dist_position = (dets[idx, 0], dets[idx, 3] + offset+180)
                combine_dist_position = (dets[idx, 0], dets[idx, 3] + offset+80)
                ...
                if len(dists) > 0 and dists is not None:
                    ...
                    cv2.putText(show_img, str(mul_dist[dist_ct]), combine_dist_position, cv2.FONT_HERSHEY_SIMPLEX, self.fontSize*0.5, (255, 255, 255),thickness=2)
                    ...
                    cv2.putText(show_img, str(mul_dist[dist_ct]), mul_dist_position, cv2.FONT_HERSHEY_SIMPLEX, self.fontSize*0.5, (255, 0, 255),thickness=2)
                    cv2.putText(show_img, str(combine_dist), combine_dist_position, cv2.FONT_HERSHEY_SIMPLEX, self.fontSize*0.5, (255, 255, 255),thickness=2)
                    dist_ct+=1
            # Add object ID
            if is_tracker:
