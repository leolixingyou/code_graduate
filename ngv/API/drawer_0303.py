...
class Drawer(object):
    ...
    def draw(self, show_img, dets, labels, dists, lidar_dist, fps, is_tracker=True):
        ...
        for idx in range(dets.shape[0]):
            ...
            if len(dists) > 0 and dists != None and lidar_dist != None:
                ...
                cv2.putText(show_img, Dist_str, (int(0.01 * w), inform_size[0][1] + int(0.01 * h)+(dist_ct-1*25)+50), self.fontFace, self.fontSize*0.5, (255, 255, 255), self.thickness)
