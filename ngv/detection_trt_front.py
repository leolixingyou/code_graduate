...
if __name__ == '__main__':
    ...
    # engine_path = "./weight/0323"#./weight/Yolov4_epoch231"
    # engine_path = "./weight/yolov4_1120_sw.trt"
    input_size = (320, 320)
    ...
    rospy.Subscriber('/gmsl_camera/port_0/cam_0/image_raw/compressed', CompressedImage, Front_IMGcallback)
    ...
    # ## 0307written
    rospy.Subscriber('/Vehicle/state',Bool,State_callback)

    pub_img = rospy.Publisher('/detection_front/result_front', Image, queue_size=10)
    pub_dist = rospy.Publisher("/detection_front/dist", Float32MultiArray, queue_size=1)
    pub_ori_dist = rospy.Publisher("/detection_front/ori_dist", Float32MultiArray, queue_size=1)
    pub_3d_bbox = rospy.Publisher('/detection_front/bbox_3d', Float32MultiArray, queue_size=1)
    ...
    with get_engine(engine_path) as engine, engine.create_execution_context() as context:
        ...
        try:
            ...
            while not rospy.is_shutdown():
                # start1 = time.time()
                # print(get_new_IMG_msg, get_new_imu_msg)
                if get_new_IMG_msg and get_new_imu_msg:
                    ...
                    if abs(math.degrees(cur_pose[1])) > 0.3 and state is None:
                        show_frame = drawer.draw(orig_im, dets_arr, labels_arr, dist_arr, GT_dist, (1. / (moving_det + 1e-8)), is_tracker=False)
                    else:
                        track_arr = tracker.update(dets_arr, labels_arr, is_dect=is_dect)
                        # show_frame = drawer.draw(orig_im, track_arr, labels_arr, dist_arr, GT_dist, (1. / (moving_det + 1e-8)), is_tracker=True)
                        show_frame = drawer.draw(orig_im, track_arr, labels_arr, dist_arr, GT_dist, (1. / (moving_det + 1e-8)), is_tracker=True)
...
#################################### draw grid ################
                        ...
                        for i in range(number_grids):   
                            # print(sself.calib.src_rightline_2d)
                            show_frame = cv2.line(show_frame, (int(calib.src_leftline_2d[i][0]), int(calib.src_leftline_2d[i][1])), \
                                (int(calib.src_rightline_2d[i][0]), int(calib.src_rightline_2d[i][1])), (255,0,0), thickness=1)
                            dist = int(i*grid_size)
                            show_frame = cv2.putText(show_frame, str(dist), (int(calib.src_leftline_2d[i][0]), int(calib.src_leftline_2d[i][1])),\
...
#######################################################################
                    ...
                    # cv2.imshow("bias_test", show_frame)
                    # key = cv2.waitKey(30)
                    # if key == 27:
                    #     break
