...
class Calibration:
    def __init__(self, camera_path, imu_cam_path, LiDAR_cam_path):
        ...
        # self.pt1_3 = [roi_x, roi_y]  # left top
        # self.pt2_3 = [roi_x, -roi_y] # right top
        # self.pt3_3 = [4.0, -roi_y]  # right bottom 
        # self.pt4_3 = [4.0, roi_y]   # left bottom
...
###################### grid draw ###########################
        ...
        pts_leftline_2d = self.project_3d_to_2d(self.pts_leftline_3d.transpose())
        pts_rightline_2d = self.project_3d_to_2d(self.pts_rightline_3d.transpose())
...
###############################################################
        ...
        pts_2d = self.project_3d_to_2d(self.pts_3d.transpose())

        self.pts_2d = pts_2d

        tem_pts = pts_2d.shape[1]
        temp = np.vstack((pts_2d, np.ones((1, tem_pts))))

        left_pts_2d = np.dot(self.Left_camera_RT, temp)
        left_pts_2d[:2, :] /= left_pts_2d[2, :]
        right_pts_2d = np.dot(self.Right_camera_RT, temp)
        right_pts_2d[:2, :] /= right_pts_2d[2, :]
    ...
    def project_3d_to_2d(self, points):
        ...
        # front_M_scale = 1.02
        # points = np.vstack((points, np.ones((1, num_pts))*front_M_scale))
        ...
        points[:2, :] /= points[2, :]
        # if task == 'left':
        #     points = np.dot(self.Left_camera_RT, points)
        #     print('points')
        #     print(points)
        # elif task == 'right':
        #     points = np.dot(self.Right_camera_RT, points)
