...
class YOLOv4_Det:
    ...
    def __init__(self, args, image_shape):
        ...
        # rospy.Subscriber('/cam1/Raw', Image, self.Right_IMGcallback)
        ...
        self.pub_img = rospy.Publisher('/detection/result3', Image, queue_size=1)
        self.pub_dist = rospy.Publisher("/detection_right/dist", Float32MultiArray, queue_size=1)
        self.pub_ori_dist = rospy.Publisher("/detection_right/ori_dist", Float32MultiArray, queue_size=1)
    ...
    #     self.cur_left_img['img'] = self.calib.undistort(left_img)
    ...
    def Right_IMGcallback(self, msg):
        if not self.get_new_IMG_msg:
            ...
            # right_img = self.bridge.imgmsg_to_cv2(msg, "bgr8")
    ...
    def main(self):
        try:
            ...
            # left_line = self.calib.src_leftline_2d_r
            # right_line = self.calib.src_rightline_2d_r
            ...
            while not rospy.is_shutdown():
                if  self.get_new_IMG_msg:
                    ...
                    if np.mod(frame_ind, self.args.interval) == 0:
                        orig_im = copy.copy(self.cur_right_img['img'])

                        # Network(YOLOv4) Input Image
                        img = cv2.resize(self.cur_right_img['img'], (320, 320))
                        ...
                        if len(bbox) > 0 and self.cur_pose is not None: 
                            ...
                            #####
                            # _,self.dist_arr, pts_3d, ground_plane, new_dist_flag, left_line, right_line  = self.est_dist.getDistance(dets_arr, self.cur_pose, 'right')
                            self.ori_dist_arr,self.dist_arr, pts_3d, ground_plane, new_dist_flag = self.est_dist.getDistance(dets_arr, self.cur_pose, 'right')
...
# ######################################################################
                    ...
                    if len(dets_arr) > 0:
                        bbox_array_msg = self.get_bbox_array_msg(dets_arr, labels_arr, self.cur_right_img['header'])

                    if self.pub_img.get_num_connections() > 0:
                        msg = None
                        try:
                            msg = self.bridge.cv2_to_imgmsg(show_frame, "bgr8")
                            msg.header = self.cur_right_img['header']
