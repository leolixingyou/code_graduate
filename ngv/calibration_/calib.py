...
class Calibration:
    def __init__(self, camera_path, imu_cam_path, LiDAR_cam_path):
        ...
        self.proj_lidar2cam_4X4 =np.array([[self.lidar_RT[0][0], self.lidar_RT[0][1], self.lidar_RT[0][2], self.lidar_RT[0][3]],
                                            [self.lidar_RT[1][0], self.lidar_RT[1][1], self.lidar_RT[1][2],self.lidar_RT[1][3]],
                                            [self.lidar_RT[2][0], self.lidar_RT[2][1], self.lidar_RT[2][2], self.lidar_RT[2][3]],
                                            [0, 0, 0, 1]])
        '''Side'''
        self.proj_lidar2left_cam = self.camera_transform(self.Left_camera_matrix, self.Left_camera_RT)
        self.proj_lidar2left_cam = np.dot(self.proj_lidar2left_cam, self.proj_lidar2cam_4X4)
        self.proj_lidar2right_cam = self.camera_transform(self.Right_camera_matrix, self.Right_camera_RT)
        self.proj_lidar2right_cam = np.dot(self.proj_lidar2right_cam, self.proj_lidar2cam_4X4)
        ...
        # Real world
        self.pt1_2 = [50.0, 3]  # left top
        self.pt2_2 = [50.0, -3] # right top
        self.pt3_2 = [4.0, -3]  # right bottom 
        self.pt4_2 = [4.0, 3]   # left bottom
...
###################### grid draw ###########################
        ...
        for i in range(number_grids):
            pts_leftline = [i*grid_size, 3, self.plane.get_z(i*grid_size, 3)]
            pts_rightline = [i*grid_size, -3, self.plane.get_z(i*grid_size, -3)]
...
###############################################################
    ...
    def camera_transform(self, intrinsic, extrinsic):
        # cam_R = np.array([[extrinsic[0][0], extrinsic[0][1], extrinsic[0][2], extrinsic[0][3]],
        ...
        # cam_transform_mat = np.dot(self.lidar_RT, cam_R)

        # return np.dot(intrinsic, cam_transform_mat)
        return np.dot(intrinsic, cam_R)
