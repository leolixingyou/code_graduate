...
def Left_IMGcallback(msg):
    ...
    if not get_new_IMG_msg:
        ...
        cur_img['img'] = calib.undistort(img, 'left')
...
if __name__ == '__main__':
    ...
    rospy.init_node('detection_left')
    rospy.Subscriber('/cam0/compressed', CompressedImage, Left_IMGcallback)
    ...
    pub_img = rospy.Publisher('/detection_left/result_left', Image, queue_size=1)
    pub_dist = rospy.Publisher("/detection_left/dist", Float32MultiArray, queue_size=1)
    pub_ori_dist = rospy.Publisher("/detection_left/ori_dist", Float32MultiArray, queue_size=1)
    pub_3d_bbox = rospy.Publisher('/detection_left/bbox_3d', Float32MultiArray, queue_size=1)
                ...
                if get_new_IMG_msg:
                    ...
                    if np.mod(frame_ind, interval) == 0:
                        ...
                        if len(bbox) > 0: 
                            ...
                            ori_dist_arr, dist_arr, pts_3d, ground_plane, new_dist_flag = est_dist.getDistance(dets_arr, cur_pose, 'left')
                            #####
